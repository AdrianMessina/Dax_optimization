...
def generate_variable_suggestion(parsed: ParsedDaxExpression) -> Suggestion:
    ...
    # Buscar patrones repetidos de CALCULATE
    calculate_matches = re.findall(r'CALCULATE\s*\([^)]+\)', parsed.raw, re.IGNORECASE)
    if calculate_matches:
        first_calc = calculate_matches[0]
        if len(first_calc) > 60:
            first_calc = first_calc[:60] + '...'
