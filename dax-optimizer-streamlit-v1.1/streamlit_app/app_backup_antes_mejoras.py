...
# Estilos CSS personalizados basados en el ícono de la aplicación
st.markdown("""
<style>
    .main-header {
        font-size: 3rem;
...
def render_header():
    ...
    st.markdown('<h1 class="main-header">⚡ DAX Optimizer v1.1</h1>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Análisis avanzado de medidas DAX con sistema de tolerancia</p>', unsafe_allow_html=True)
...
def export_measures_to_excel(ranked_measures):
    """Exporta las medidas analizadas a un archivo Excel con formato"""
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df.to_excel(writer, index=False, sheet_name='Análisis DAX')

        # Ajustar anchos de columna
        worksheet = writer.sheets['Análisis DAX']
...
def analyze_pbip_file(file_path: str):
    ...
    with st.spinner("🔄 Iniciando análisis del archivo PBIP..."):
        time.sleep(0.8)  # Pequeño delay para dar sensación de análisis
        ...
        try:
            ...
            if ranked_measures:
                ...
                if EXCEL_AVAILABLE:
                    ...
                    col_export1, col_export3 = st.columns([1, 5])
                ...
                if EXCEL_AVAILABLE:
                    with col_export2:
                        excel_data = export_measures_to_excel(ranked_measures)
                        st.download_button(
                            label="📥 Exportar Excel",
                            data=excel_data,
                            file_name=f"dax_analysis_{time.strftime('%Y%m%d_%H%M%S')}.xlsx",
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            help="Descargar análisis completo en formato Excel con formato"
                        )
                else:
                    with col_export3:
